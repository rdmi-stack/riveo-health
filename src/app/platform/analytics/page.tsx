import Link from "next/link";
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";
import {
  ArrowRight,
  ArrowLeft,
  LayoutDashboard,
  Search,
  TrendingUp,
  Users,
  LineChart,
  BarChart3,
  PieChart,
  Activity,
  Clock,
  FileText,
  DollarSign,
} from "lucide-react";

export const metadata = {
  title: "Analytics & Insights | Riveo Health",
  description:
    "See where every dollar goes and where it doesn't. Real-time dashboards, revenue leakage detection, payer performance, and staff productivity analytics.",
};

const features = [
  {
    icon: LayoutDashboard,
    title: "Real-time Dashboards",
    desc: "Live visibility into claims status, revenue flow, denial trends, and collection performance across your entire organization.",
    gradient: "from-indigo-500 to-blue-500",
    bg: "bg-indigo-50",
    tc: "text-indigo-600",
    border: "border-indigo-100",
  },
  {
    icon: Search,
    title: "Revenue Leakage Detection",
    desc: "AI continuously scans for missed charges, under-coded encounters, unbilled services, and collection gaps that silently erode your revenue.",
    gradient: "from-cyan-500 to-teal-500",
    bg: "bg-cyan-50",
    tc: "text-cyan-600",
    border: "border-cyan-100",
  },
  {
    icon: TrendingUp,
    title: "Payer Performance",
    desc: "Benchmark payer behavior across reimbursement rates, denial patterns, turnaround times, and contract compliance to negotiate from a position of strength.",
    gradient: "from-violet-500 to-purple-500",
    bg: "bg-violet-50",
    tc: "text-violet-600",
    border: "border-violet-100",
  },
  {
    icon: Users,
    title: "Staff Productivity",
    desc: "Track team performance, workload distribution, and efficiency metrics to optimize resource allocation and identify coaching opportunities.",
    gradient: "from-amber-500 to-orange-500",
    bg: "bg-amber-50",
    tc: "text-amber-600",
    border: "border-amber-100",
  },
];

const reportTypes = [
  { icon: LineChart, label: "Revenue Trends", desc: "Track revenue over time with drill-down by payer, provider, or location" },
  { icon: BarChart3, label: "Denial Analysis", desc: "Break down denials by reason code, payer, and department" },
  { icon: PieChart, label: "Payer Mix", desc: "Visualize revenue distribution across commercial, Medicare, and Medicaid" },
  { icon: Activity, label: "Claim Lifecycle", desc: "Monitor average days from charge to payment at every stage" },
];

const metrics = [
  { value: "Real-time", label: "Data refresh", icon: Clock },
  { value: "50+", label: "Pre-built reports", icon: FileText },
  { value: "$78K", label: "Avg leakage found", icon: DollarSign },
];

export default function AnalyticsPage() {
  return (
    <>
      <Navbar />
      <main>
        {/* ===== HERO ===== */}
        <section className="relative pt-24 overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900" />
          <div
            className="absolute inset-0 opacity-[0.03]"
            style={{
              backgroundImage:
                "linear-gradient(rgba(255,255,255,0.5) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.5) 1px, transparent 1px)",
              backgroundSize: "60px 60px",
            }}
          />
          <div className="absolute top-20 right-0 w-96 h-96 bg-amber-500/10 rounded-full blur-3xl" />
          <div className="absolute bottom-0 left-20 w-80 h-80 bg-indigo-500/10 rounded-full blur-3xl" />

          <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-32 text-center">
            <Link
              href="/platform"
              className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-cyan-400 transition-colors mb-8"
            >
              <ArrowLeft className="w-4 h-4" />
              Back to Platform
            </Link>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 border border-white/10 mb-8">
              <LineChart className="w-4 h-4 text-amber-400" />
              <span className="text-sm font-medium text-amber-300">Analytics & Insights</span>
            </div>
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-[1.1] tracking-tight">
              See where every dollar goes.{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-300 to-orange-300">And where it doesn&apos;t.</span>
            </h1>
            <p className="mt-6 text-lg text-slate-300 leading-relaxed max-w-2xl mx-auto">
              Actionable analytics that surface hidden revenue opportunities,
              track performance in real time, and give your leadership team the
              data they need to make confident decisions.
            </p>
          </div>
        </section>

        {/* ===== FEATURES ===== */}
        <section className="py-24 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-3xl mx-auto mb-16">
              <p className="text-sm font-semibold text-primary uppercase tracking-widest mb-4">
                Capabilities
              </p>
              <h2 className="text-3xl sm:text-4xl font-bold text-surface-dark tracking-tight">
                Complete revenue visibility
              </h2>
              <p className="mt-4 text-text-secondary leading-relaxed">
                Four dimensions of analytics that give your leadership team a complete picture of financial performance.
              </p>
            </div>

            <div className="grid sm:grid-cols-2 gap-8">
              {features.map((feat) => {
                const Icon = feat.icon;
                return (
                  <div
                    key={feat.title}
                    className={`group relative bg-white rounded-2xl p-8 border ${feat.border} hover:shadow-xl transition-all duration-300 overflow-hidden`}
                  >
                    <div className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${feat.gradient}`} />
                    <div className="flex items-start gap-5">
                      <div
                        className={`w-14 h-14 rounded-2xl ${feat.bg} flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform`}
                      >
                        <Icon className={`w-7 h-7 ${feat.tc}`} />
                      </div>
                      <div>
                        <h3 className="text-xl font-bold text-surface-dark mb-3">
                          {feat.title}
                        </h3>
                        <p className="text-text-secondary leading-relaxed">
                          {feat.desc}
                        </p>
                      </div>
                    </div>
                  </div>
                );
              })}
            </div>
          </div>
        </section>

        {/* ===== REPORT TYPES ===== */}
        <section className="py-24 bg-slate-50">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-3xl mx-auto mb-16">
              <p className="text-sm font-semibold text-primary uppercase tracking-widest mb-4">
                Report Library
              </p>
              <h2 className="text-3xl sm:text-4xl font-bold text-surface-dark tracking-tight">
                50+ pre-built reports, ready to go
              </h2>
            </div>

            <div className="grid sm:grid-cols-2 gap-6">
              {reportTypes.map((report) => {
                const Icon = report.icon;
                return (
                  <div key={report.label} className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm hover:shadow-lg transition-all group">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform">
                        <Icon className="w-6 h-6 text-white" />
                      </div>
                      <div>
                        <h3 className="text-lg font-bold text-surface-dark mb-1">{report.label}</h3>
                        <p className="text-sm text-text-secondary leading-relaxed">{report.desc}</p>
                      </div>
                    </div>
                  </div>
                );
              })}
            </div>
          </div>
        </section>

        {/* ===== METRICS ===== */}
        <section className="py-20 bg-white">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid sm:grid-cols-3 gap-8">
              {metrics.map((m) => {
                const Icon = m.icon;
                return (
                  <div
                    key={m.label}
                    className="relative text-center p-8 rounded-2xl bg-slate-50 border border-gray-100 overflow-hidden group hover:shadow-lg transition-all"
                  >
                    <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-amber-500 to-orange-500 opacity-0 group-hover:opacity-100 transition-opacity" />
                    <div className="w-10 h-10 rounded-xl gradient-bg flex items-center justify-center mx-auto mb-4">
                      <Icon className="w-5 h-5 text-white" />
                    </div>
                    <p className="text-4xl sm:text-5xl font-bold gradient-text mb-2">
                      {m.value}
                    </p>
                    <p className="text-text-secondary font-medium">{m.label}</p>
                  </div>
                );
              })}
            </div>
          </div>
        </section>

        {/* ===== CTA ===== */}
        <section className="py-24 bg-gradient-to-br from-indigo-600 via-indigo-700 to-slate-900 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-96 h-96 bg-cyan-400/10 rounded-full blur-3xl" />
          <div className="absolute bottom-0 left-0 w-80 h-80 bg-indigo-400/10 rounded-full blur-3xl" />

          <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h2 className="text-4xl sm:text-5xl font-bold text-white tracking-tight leading-tight">
              Unlock hidden revenue
            </h2>
            <p className="mt-6 text-lg text-indigo-200 max-w-2xl mx-auto leading-relaxed">
              See how Analytics & Insights can surface the revenue opportunities
              your current tools are missing.
            </p>
            <div className="mt-10">
              <Link
                href="/demo"
                className="group inline-flex items-center justify-center gap-2 px-8 py-4 text-base font-semibold text-indigo-700 bg-white rounded-full hover:bg-indigo-50 transition-colors shadow-xl"
              >
                Request a Demo
                <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </Link>
            </div>
          </div>
        </section>
      </main>
      <Footer />
    </>
  );
}
