import Link from "next/link";
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";
import {
  ArrowRight,
  ArrowLeft,
  Building2,
  Globe,
  CreditCard,
  FileCode2,
  Plug,
  CheckCircle2,
  Clock,
  Shield,
  Server,
} from "lucide-react";

export const metadata = {
  title: "Integrations | Riveo Health",
  description:
    "Plugs into what you already use. Pre-built connectors for Epic, Cerner, athena, eCW, clearinghouses, payer APIs, and payment gateways via HL7/FHIR.",
};

const features = [
  {
    icon: Building2,
    title: "EHR & Practice Management",
    desc: "Pre-built, certified connectors for Epic, Cerner, athenahealth, eClinicalWorks, NextGen, and other major EHR and practice management systems.",
    gradient: "from-indigo-500 to-blue-500",
    bg: "bg-indigo-50",
    tc: "text-indigo-600",
    border: "border-indigo-100",
  },
  {
    icon: Globe,
    title: "Clearinghouses & Payer APIs",
    desc: "Direct connections to major clearinghouses and payer portals for real-time eligibility checks, claim status inquiries, and ERA/EOB retrieval.",
    gradient: "from-cyan-500 to-teal-500",
    bg: "bg-cyan-50",
    tc: "text-cyan-600",
    border: "border-cyan-100",
  },
  {
    icon: CreditCard,
    title: "Payment Gateways",
    desc: "Integrated payment processing for patient collections, including credit/debit, ACH, digital wallets, and flexible payment plan management.",
    gradient: "from-violet-500 to-purple-500",
    bg: "bg-violet-50",
    tc: "text-violet-600",
    border: "border-violet-100",
  },
  {
    icon: FileCode2,
    title: "HL7/FHIR Standards",
    desc: "Full support for HL7v2, FHIR R4, and X12 EDI standards ensuring interoperability with any compliant healthcare system.",
    gradient: "from-amber-500 to-orange-500",
    bg: "bg-amber-50",
    tc: "text-amber-600",
    border: "border-amber-100",
  },
];

const supportedSystems = [
  {
    category: "EHR / PM Systems",
    systems: ["Epic", "Cerner (Oracle Health)", "athenahealth", "eClinicalWorks", "NextGen Healthcare", "Allscripts", "MEDITECH", "DrChrono"],
    color: "from-indigo-500 to-blue-500",
    icon: Building2,
  },
  {
    category: "Clearinghouses",
    systems: ["Availity", "Change Healthcare", "Trizetto", "Waystar", "Office Ally", "ClaimMD"],
    color: "from-cyan-500 to-teal-500",
    icon: Globe,
  },
  {
    category: "Payer Portals",
    systems: ["UnitedHealthcare", "Anthem / Elevance", "Aetna / CVS Health", "Cigna", "Humana", "Medicare / Medicaid"],
    color: "from-violet-500 to-purple-500",
    icon: Shield,
  },
  {
    category: "Payment Platforms",
    systems: ["Stripe", "Square", "PayPal Health", "InstaMed", "PatientPay", "Flywire"],
    color: "from-amber-500 to-orange-500",
    icon: CreditCard,
  },
];

const metrics = [
  { value: "40+", label: "Pre-built integrations", icon: Plug },
  { value: "<7 days", label: "Average setup time", icon: Clock },
  { value: "99.9%", label: "Uptime SLA", icon: Server },
];

export default function IntegrationsPage() {
  return (
    <>
      <Navbar />
      <main>
        {/* ===== HERO ===== */}
        <section className="relative pt-24 overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900" />
          <div
            className="absolute inset-0 opacity-[0.03]"
            style={{
              backgroundImage:
                "linear-gradient(rgba(255,255,255,0.5) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.5) 1px, transparent 1px)",
              backgroundSize: "60px 60px",
            }}
          />
          <div className="absolute top-20 right-0 w-96 h-96 bg-emerald-500/10 rounded-full blur-3xl" />
          <div className="absolute bottom-0 left-20 w-80 h-80 bg-indigo-500/10 rounded-full blur-3xl" />

          <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-32 text-center">
            <Link
              href="/platform"
              className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-cyan-400 transition-colors mb-8"
            >
              <ArrowLeft className="w-4 h-4" />
              Back to Platform
            </Link>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 border border-white/10 mb-8">
              <Plug className="w-4 h-4 text-emerald-400" />
              <span className="text-sm font-medium text-emerald-300">Integrations</span>
            </div>
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-[1.1] tracking-tight">
              Plugs into what you already use.{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-300 to-cyan-300">No rip-and-replace.</span>
            </h1>
            <p className="mt-6 text-lg text-slate-300 leading-relaxed max-w-2xl mx-auto">
              Riveo Health connects to your existing EHR, practice management
              system, clearinghouses, and payment infrastructure in days, not
              months.
            </p>
          </div>
        </section>

        {/* ===== FEATURES ===== */}
        <section className="py-24 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-3xl mx-auto mb-16">
              <p className="text-sm font-semibold text-primary uppercase tracking-widest mb-4">
                Integration Categories
              </p>
              <h2 className="text-3xl sm:text-4xl font-bold text-surface-dark tracking-tight">
                Connect everything. Disrupt nothing.
              </h2>
              <p className="mt-4 text-text-secondary leading-relaxed">
                Four categories of pre-built connectors that bridge Riveo Health with your existing infrastructure.
              </p>
            </div>

            <div className="grid sm:grid-cols-2 gap-8">
              {features.map((feat) => {
                const Icon = feat.icon;
                return (
                  <div
                    key={feat.title}
                    className={`group relative bg-white rounded-2xl p-8 border ${feat.border} hover:shadow-xl transition-all duration-300 overflow-hidden`}
                  >
                    <div className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${feat.gradient}`} />
                    <div className="flex items-start gap-5">
                      <div
                        className={`w-14 h-14 rounded-2xl ${feat.bg} flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform`}
                      >
                        <Icon className={`w-7 h-7 ${feat.tc}`} />
                      </div>
                      <div>
                        <h3 className="text-xl font-bold text-surface-dark mb-3">
                          {feat.title}
                        </h3>
                        <p className="text-text-secondary leading-relaxed">
                          {feat.desc}
                        </p>
                      </div>
                    </div>
                  </div>
                );
              })}
            </div>
          </div>
        </section>

        {/* ===== SUPPORTED SYSTEMS ===== */}
        <section className="py-24 bg-slate-50">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-3xl mx-auto mb-16">
              <p className="text-sm font-semibold text-primary uppercase tracking-widest mb-4">
                Supported Systems
              </p>
              <h2 className="text-3xl sm:text-4xl font-bold text-surface-dark tracking-tight">
                Works with the tools you rely on
              </h2>
            </div>

            <div className="grid sm:grid-cols-2 gap-8">
              {supportedSystems.map((group) => {
                const Icon = group.icon;
                return (
                  <div key={group.category} className="bg-white rounded-2xl p-8 border border-gray-200 shadow-sm hover:shadow-lg transition-all">
                    <div className="flex items-center gap-3 mb-6">
                      <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${group.color} flex items-center justify-center`}>
                        <Icon className="w-5 h-5 text-white" />
                      </div>
                      <h3 className="text-lg font-bold text-surface-dark">{group.category}</h3>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {group.systems.map((system) => (
                        <div
                          key={system}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-50 border border-gray-100 text-sm font-medium text-surface-dark"
                        >
                          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />
                          {system}
                        </div>
                      ))}
                    </div>
                  </div>
                );
              })}
            </div>
          </div>
        </section>

        {/* ===== METRICS ===== */}
        <section className="py-20 bg-white">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid sm:grid-cols-3 gap-8">
              {metrics.map((m) => {
                const Icon = m.icon;
                return (
                  <div
                    key={m.label}
                    className="relative text-center p-8 rounded-2xl bg-slate-50 border border-gray-100 overflow-hidden group hover:shadow-lg transition-all"
                  >
                    <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-emerald-500 to-green-500 opacity-0 group-hover:opacity-100 transition-opacity" />
                    <div className="w-10 h-10 rounded-xl gradient-bg flex items-center justify-center mx-auto mb-4">
                      <Icon className="w-5 h-5 text-white" />
                    </div>
                    <p className="text-4xl sm:text-5xl font-bold gradient-text mb-2">
                      {m.value}
                    </p>
                    <p className="text-text-secondary font-medium">{m.label}</p>
                  </div>
                );
              })}
            </div>
          </div>
        </section>

        {/* ===== CTA ===== */}
        <section className="py-24 bg-gradient-to-br from-indigo-600 via-indigo-700 to-slate-900 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-96 h-96 bg-cyan-400/10 rounded-full blur-3xl" />
          <div className="absolute bottom-0 left-0 w-80 h-80 bg-indigo-400/10 rounded-full blur-3xl" />

          <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h2 className="text-4xl sm:text-5xl font-bold text-white tracking-tight leading-tight">
              Ready to connect?
            </h2>
            <p className="mt-6 text-lg text-indigo-200 max-w-2xl mx-auto leading-relaxed">
              See how Riveo Health integrates with your existing systems in under
              7 days with zero disruption to your workflows.
            </p>
            <div className="mt-10">
              <Link
                href="/demo"
                className="group inline-flex items-center justify-center gap-2 px-8 py-4 text-base font-semibold text-indigo-700 bg-white rounded-full hover:bg-indigo-50 transition-colors shadow-xl"
              >
                Request a Demo
                <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </Link>
            </div>
          </div>
        </section>
      </main>
      <Footer />
    </>
  );
}
